...
class QADataset(Dataset):
    """QA问答数据集加载器（适配instruction+output字段）"""
    ...
    def _parse_qa_dict(self, data):
        """解析QA字典（提取instruction和output并组装）"""
        # 提取instruction和output字段（兼容大小写）
        question = data.get('instruction') or data.get('Instruction')
        answer = data.get('output') or data.get('Output')
...
def qa_finetuning():
    ...
    config = {
        "data_dir": "./QA-data-3",  # 修改为新的问答数据目录
        "output_dir": "./qwen2.5-0.5b-io-lora-r16",  # 输出目录（可自定义）
