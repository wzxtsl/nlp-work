...
class QADataset(Dataset):
    """QA问答数据集加载器（适配prompt+response字段）"""
    ...
    def _parse_qa_dict(self, data):
        """解析QA字典（提取prompt和response并组装）"""
        # 提取prompt和response字段（兼容大小写）
        question = data.get('prompt') or data.get('Prompt')
        answer = data.get('response') or data.get('Response')
...
def qa_finetuning():
    ...
    config = {
        "data_dir": "./QA-data-2",  # 修改为新的问答数据目录
        "output_dir": "./qwen2.5-0.5b-pr-lora-r16",  # 输出目录（可自定义）
